...
# robust_downloader is only needed for downloading the full model
# We only use the bundled lite model, so make it optional
try:
...
class ModelDownloader:
    ...
    def download(url: str, save_path: Path, proxy: Optional[str] = None) -> None:
        ...
        :raises:
            FastLangdetectError: If download fails or downloader not available
            FileNotFoundError: If a user-provided cache directory does not exist
        """
        if download is None:
            raise FastLangdetectError(
                "fast-langdetect: Full model download requires robust_downloader package. "
                "Use model='lite' instead."
            )
